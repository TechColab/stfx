...
if [ "$bus" != "" ] ; then
  ...
  if( addr == 0+"0x20" ) { $4 = "MCP23017 16-pin I/O expander" }
  # if( addr == 0+"0x20" ) { $4 = "Adafruit LCD + buttons" }
  if( addr == 0+"0x3b" ) { $4 = "Raspberry Pi camera" }
  if( addr == 0+"0x40" ) { $4 = "Adafruit 16-ch PWM PCA9685" }
  if( addr == 0+"0x48" ) { $4 = "Adafruit 4-ch ADC ADS1015/ADS1115" }
  if( addr == 0+"0x68" ) { $4 = "DS1307 RTC" }
  if( addr == 0+"0x70" ) { $4 = "Built-in 4-ch I2C multiplexer" }
  if( addr == 0+"0x77" ) { $4 = "Bosch BMP085 pressure & temperature" }
