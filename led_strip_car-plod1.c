/*
    led_strip_car-fuzz1 - flash RGB LED strip like a UK police car.
...
int main(int argc, char **argv) {
  ...
  loop_seq = 0;
  for(loop_led=0; loop_led<4; loop_led+=1 ) { ary[loop_seq][(3*loop_led)+2] = 255 ; }
  for(loop_led=(32-4); loop_led<32; loop_led+=1 ) { ary[loop_seq][(3*loop_led)+2] = 255 ; }

  // animate the sequence
  for(loop_seq=0; loop_seq<10; loop_seq++) {
